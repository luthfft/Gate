...
class ApiController {
  ...
  async sendMessage(req, res, next) {
    try {
      ...
      if (!receiver || !apikey || !message_type)
        throw new Validator("Invalid parameters.", 400);

      let settings = fs.existsSync("storage/json/settings.json", "utf8")
        ? JSON.parse(fs.readFileSync("storage/json/settings.json", "utf8"))
        : {};
      let device = Object.keys(settings).find(
        (device) => settings[device].apikey == apikey
      );
      ...
      switch (message_type) {
        ...
        case "document":
          ...
          var document = fs.readFileSync(url);
          if (!document) throw new Validator("Invalid parameters.", 400);
          filename = filename || document.split("/").pop();
          document = await axois
            .get(document, { responseType: "arraybuffer" })
            .catch((error) => {
              throw new Validator("Invalid document url.", 400);
            });
  ...
  async fetchGroup(req, res, next) {
    try {
      ...
      return res
        .status(200)
        .json({ status: true, message: "Fetched groups.", data: groups });
    } catch (error) {
      return res.status(error.code || 500).json({
        status: false,
        message: error.message || "Internal server error.",
  ...
  async fetchMember(req, res, next) {
    try {
      ...
      return res.status(200).json({
        status: true,
        message: "Fetched groups " + group.subject,
